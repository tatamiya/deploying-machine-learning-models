...
def validate_inputs(input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    if input_data[config.NUMERICAL_NA_NOT_ALLOWED].isnull().any().any():
        validated_data = validated_data.dropna(subset=config.NUMERICAL_NA_NOT_ALLOWED, axis=0)
    ...
    if input_data[config.CATEGORICAL_NA_NOT_ALLOWED].isnull().any().any():
        validated_data = validated_data.dropna(subset=config.CATEGORICAL_NA_NOT_ALLOWED, axis=0)

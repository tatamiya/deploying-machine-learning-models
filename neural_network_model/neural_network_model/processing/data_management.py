...
def load_single_image(data_folder: str, filename: str) -> pd.DataFrame:
    ...
    for image_path in glob(os.path.join(data_folder, f'{filename}')):
        ...
        list_image_df.append(tmp)
    
    # concatenate the final df
    images_df = pd.concat(list_image_df, axis=0, ignore_index=True)
...
def load_image_paths(data_folder: str) -> pd.DataFrame:
    ...
    for class_folder_name in os.listdir(data_folder):
        ...
        for image_path in glob(os.path.join(class_folder_path, "*.png")):
            ...
            list_images_df.append(tmp)
    
    # concatinate the final df
    images_df = pd.concat(list_images_df, axis=0, ignore_index=True)
    images_df.columns = ['image', 'target']
...
def save_pipeline_keras(model) -> None:
    ...
    joblib.dump(model.named_steps['dataset'], config.PIPELIEN_PATH)
...
def load_pipelien_keras() -> Pipeline:
    """
    Load a Keras Pipeline from disk.
    """
    
    dataset = joblib.load(config.PIPELIEN_PATH)
...
def remove_old_pipelines(*, files_to_keep: t.List[str]) -> None:
    """
    Remove old model pipelines, model, encoders and classes.

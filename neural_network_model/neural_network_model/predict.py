...
def make_single_prediction(*, image_name: str, image_directory: str):
    """Make a single prediction uisng the saved model pipeline.
...
def make_bulk_predictions(*, image_df: pd.Series) -> dict:
    """
    Make multiple predictions using the saved model pipeline.
    
    Currently, this function is primarily for testing purpose,
    allowing us to pass in a directory of images for running
    bulk predictions.
    
    Args:
        image_df: Pandas series of images
    
    Returns:
        Dictionary with both raw predictions and their classifications.
    """
    
    _logger.info(f'received input df: {image_df}')
    
    predictions = KERAS_PIPELINE.predict(image_df)
    readable_predictions = ENCODER.encoder.inverse_transform(predictions)
